...
describe("Pix.payload", () => {
  it("genera un BR Code con CRC válido", () => {
    ...
    expect(payload).toContain("5303986"); // moneda BRL
    expect(payload).toContain("5802BR");
  });

  it("formatea el monto con 2 decimales", () => {
  ...
  it("omite el monto si no se especifica (QR de monto abierto)", () => {
    const parsed = Pix.parse(Pix.payload(BASE));
    expect(parsed.amount).toBeUndefined();
  });

  it("normaliza diacríticos y recorta nombre (25) y ciudad (15)", () => {
  ...
  it("usa '***' como txid por defecto", () => {
    expect(Pix.parse(Pix.payload(BASE)).txid).toBe("***");
  });

  it("sanea el txid a alfanumérico de máx. 25", () => {
...
describe("Pix.parse / validate", () => {
  ...
  it("rechaza strings que no son BR Codes", () => {
...
describe("Pix.fromCode", () => {
  it("envuelve un copia-e-cola válido", () => {
  ...
  it("rechaza códigos con CRC inválido salvo validate:false", () => {
...
describe("PixQr render", () => {
  ...
  it("toString devuelve el payload (copia e cola)", () => {
